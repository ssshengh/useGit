...
{
	std::vector < NetDeviceContainer > groups; //groups created 生成集群
	...
	while (numTransmittersSelected < ntransmitters) //选择的发射者小于发射者总数
	  {
	      ...
	      for (uint32_t j = 0 ; j < nRxDevices; ++j)
	        {
	          ...
	          if(rx->GetNode()->GetId() != tx->GetNode()->GetId())//No loopback link possible due to half-duplex
	          {
	           ...
	           //   {
	                rsrpRx = m_lteHelper->CalcSidelinkRsrp (txPower, ulEarfcn, ulBandwidth, tx, rx); //接收者RSRP计算方式
	           //   }
           //   else
	           //   {
...
//如果接收机UE不在发射机UE的X dBm的RSRP *之内，则在发射机UE X dBm的RSRP之内并且还不属于组的UE之中随机选择接收机UE。
...
std::vector < NetDeviceContainer > 
LteV2xHelper::AssociateForV2xMulticast (NetDeviceContainer ues, uint32_t ntransmitters)
{
  std::vector < NetDeviceContainer > groups; //groups created
	  
  for (uint32_t i = 1 ; i < ntransmitters ; i++)
    {
      Ptr<NetDevice> tx = ues.Get (i);
      //Ptr<NetDevice> LastUe = ues.Get (i-1);
      //Ptr<NetDevice> NextUe = ues.Get (i+1);
      
      NetDeviceContainer newGroup (tx);
      newGroup.Add (ues.Get(0));
      if (i != 1)
      {     
         //newGroup.Add (LastUe);
		 newGroup.Add (ues.Get (i-1));
      }
      if (i!= (ntransmitters-1))
      {
         //newGroup.Add (NextUe);
		 newGroup.Add (ues.Get (i+1));
      }
...
LteV2xHelper::PrintGroups (std::vector<NetDeviceContainer> groups)
{
    ...
    for (gIt = groups.begin() ; gIt != groups.end() ; gIt++)
      {        
        ...
        for (uint32_t i = 1; i < (*gIt).GetN(); i++)  //1
          {
...
void
LteV2xHelper::PrintGroups (std::vector < NetDeviceContainer > groups, Ptr< OutputStreamWrapper > stream)  //打印connections
{
