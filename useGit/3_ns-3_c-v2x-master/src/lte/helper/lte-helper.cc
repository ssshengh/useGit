...
LteHelper::DoCalcRsrp (Ptr<PropagationLossModel> propagationLoss, double txPower, Ptr<SpectrumPhy> txPhy, Ptr<SpectrumPhy> rxPhy)
{
 ...
 /// std::cout<< "pathLossDb = " <<pathLossDb << ";";   ///////
...
//double rsrp= DoCalcRsrp (lossModel, 23.0, txDevice->GetObject<LteUeNetDevice> ()->GetPhy()->GetUlSpectrumPhy (), rxDevice->GetObject<LteUeNetDevice> ()->GetPhy()->GetUlSpectrumPhy ());
